...
_dev_fallback="dev-secret-key-change-me-not-for-production"; _secret=os.environ.get("SECRET_KEY")
if not _secret:
...
app.secret_key=_secret; app.config["SECRET_KEY"]=_secret; app.config["SESSION_COOKIE_HTTPONLY"]=True; app.config["SESSION_COOKIE_SAMESITE"]="Lax"; app.config["SESSION_COOKIE_SECURE"]=os.environ.get("SESSION_COOKIE_SECURE","").lower() in ("1","true","yes") or os.environ.get("FLASK_ENV")=="production" or os.environ.get("NEXORA_ENV")=="production"; app.config["MAX_CONTENT_LENGTH"]=5*1024*1024
...
try:
    ...
    if app.config.get("WTF_CSRF_ENABLED"): raise RuntimeError("Flask-WTF is required for CSRF protection but is not installed. Install with: pip install flask-wtf") from _csrf_err
    csrf=None
@app.after_request
...
def user_profile_picture(user_id):
    ...
    try:
        row=conn.execute("SELECT profile_picture FROM student_profiles WHERE user_id=?",(user_id,)).fetchone()
        filename=row[0] if row else None
    finally: conn.close()
    if not filename: filename="default_profile.png"; folder=BASE_DIR/"resources"/"assets"/"images"
    else: folder=PROFILE_UPLOAD_FOLDER
...
def repair_missing_student_profiles():
    ...
    try:
        if using_postgres(): cursor.execute("INSERT INTO student_profiles (user_id,profile_completed) SELECT u.id,0 FROM users u WHERE u.role='student' AND NOT EXISTS (SELECT 1 FROM student_profiles sp WHERE sp.user_id=u.id) ON CONFLICT (user_id) DO NOTHING")
        else: cursor.execute("INSERT OR IGNORE INTO student_profiles (user_id,profile_completed) SELECT u.id,0 FROM users u WHERE u.role='student' AND NOT EXISTS (SELECT 1 FROM student_profiles sp WHERE sp.user_id=u.id)")
        conn.commit()
...
app.register_blueprint(auth); app.register_blueprint(password); app.register_blueprint(student); app.register_blueprint(student_classwork); app.register_blueprint(student_gradebook); app.register_blueprint(student_classmates); app.register_blueprint(supervisor); app.register_blueprint(admin); app.register_blueprint(classroom); app.register_blueprint(classwork); app.register_blueprint(classwork_submissions); app.register_blueprint(classwork_grading); app.register_blueprint(classwork_scores); app.register_blueprint(classwork_gradebook); app.register_blueprint(classwork_gradebook_export); app.register_blueprint(classwork_ml_insights); app.register_blueprint(performance_reports); app.register_blueprint(notifications_bp)

@app.context_processor
def inject_notifications():
    if "user_id" not in session: return {"notifications":[],"recent_notifications":[],"unread_count":0,"sidebar_profile":None}
    user_id=session["user_id"]; sidebar_profile=None
    try:
        conn=get_db_connection()
        try:
            ...
            if user:
                ...
                if user["role"]=="student":
                    p=conn.execute("SELECT first_name,last_name,profile_picture FROM student_profiles WHERE user_id=?",(user_id,)).fetchone()
                    if p: sidebar_profile.update({"first_name":p["first_name"],"last_name":p["last_name"],"profile_picture":p["profile_picture"]})
        finally: conn.close()
        ...
        print("inject_notifications failed:",e); return {"notifications":[],"recent_notifications":[],"unread_count":0,"sidebar_profile":sidebar_profile}
